...
public class UpdateClassroomController implements ActionListener {	
	...
	UpdateClassroomController(){
		...
		getDetails.setBounds(65,120,200,30);
		formPanel.add(getDetails);

		labelBlock = new JLabel("BLOCK :");
		labelBlock.setBounds(65,200,86,23);
		formPanel.add(labelBlock);

		block = new JTextField();
		block.setBounds(65,225,250,20);
		formPanel.add(block);

		labelFloor = new JLabel("FLOOR :");
		labelFloor.setBounds(65,265,86,23);
		formPanel.add(labelFloor);

		floor = new JTextField();
		floor.setBounds(65,290,250,20);
		formPanel.add(floor);

		labelCapacity = new JLabel("CAPACITY :");
		labelCapacity.setBounds(65,330,86,23);
		formPanel.add(labelCapacity);

		capacity = new JTextField();
		capacity.setBounds(65,355,250,20);
		formPanel.add(capacity);

		updateDetails = new JButton("Update Class Details");
		updateDetails.setBounds(65,405,200,30);
		formPanel.add(updateDetails);

		ImageIcon background = new ImageIcon("/home/ankur/Pictures/classroom.jpg");
		labelImg = new JLabel();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == getDetails)
		{
			...
			block.setText(classData.getBlock());
			floor.setText(classData.getFloor());
			capacity.setText(classData.getCapacity());			
		}
		if (e.getSource() == updateDetails)		
		{
			...
			classData.setID(s1);
			classData.setBlock(s2);
			classData.setFloor(s3);
			classData.setCapacity(s4);
